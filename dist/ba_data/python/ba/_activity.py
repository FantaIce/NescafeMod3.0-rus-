...
from ModData.colors import actor_colors, actor_animations

def get_color(color_arg):
    if isinstance(color_arg, str):
        if color_arg == 'random':
            return (random.uniform(0.1, 1.3), random.uniform(0.1, 1.3), random.uniform(0.1, 1.3))
        elif color_arg in actor_colors:
            return actor_colors[color_arg]
        else:
            return None
    elif isinstance(color_arg, tuple) and len(color_arg) == 3:
        return color_arg
    elif isinstance(color_arg, list) and len(color_arg) == 3:
        return tuple(color_arg)
    else:
        return None

def get_animation(animation_arg):
    if isinstance(animation_arg, str):
        if animation_arg in actor_animations:
            return actor_animations[animation_arg]
        else:
            return None
    else:
        return None

class ColorNotFoundMessage:
    """Inform something that we did not find the color."""
...
class Activity(DependencyComponent, Generic[PlayerType, TeamType]):
    ...
    def set_node_color(self, node, param_str, color, k = 1.0):
        color = (k * color[0], k * color[1], k * color[2])
        try:
            if hasattr(node, param_str):
                #setattr(node, param_str, color)
                ba.animate_array(node, param_str, 3, {0.0: color, 0.0: color})
                return True
            return False
        except:
            return False

    def set_node_animation(self, node, param_str, color1, color2, time_interval, k = 1.0):
        color1 = (k * color1[0], k * color1[1], k * color1[2])
        color2 = (k * color2[0], k * color2[1], k * color2[2])
        try:
            if hasattr(node, param_str):
                ba.animate_array(node, param_str, 3, {0: color1, time_interval: color2})
                return True
            return False
        except:
            return False

    def change_node_color(self, node, param_str='color', color_arg = 'white', k = 1.0):
        self.stop_node_animation(node, param_str)
        color = get_color(color_arg)
        if color == None:
            if isinstance(color_arg, str):
                return self.start_node_animation(node, param_str, color_arg, k)
            self.handlemessage(ColorNotFoundMessage())
            return False
        return self.set_node_color(node, param_str, color, k)

    def stop_node_animation(self, node, param_str='color'):
        for animation in self.node_animations:
            if animation != None and animation['node'] == node and animation['color_parameter'] == param_str:
                animation['timer'] = None
                self.node_animations.remove(animation)
                #try:
                #    color = getattr(node, param_str)
                #    ba.animate_array(node, param_str, 3, {0.0: color, 0.0: color})
                #except:
                #    pass

    def start_node_animation(self, node, param_str='color', animation_arg='rand_an', k = 1.0):
        self.stop_node_animation(node, param_str)
        if isinstance(animation_arg, str):
            gotten_animation = get_animation(animation_arg)
            if gotten_animation == None:
                self.handlemessage(ColorNotFoundMessage())
                return
            animation = {
                'node': node,
                'color_parameter': param_str,
                'color_index': 0,
                'cur_color': None,
            ...
            animation.update(gotten_animation)
        else:
            animation = {
                'node': node,
                'color_parameter': param_str,
                'colors': animation_arg['colors'],
                'color_index': 0,
                'cur_color': None,
                'time_intervals': [0.5],
                'time_interval_index': 0,
            ...
            if 'time_intervals' in animation_arg:
                animation['time_intervals'] = animation_arg['time_intervals']
            if 'gradient' in animation_arg:
                animation['gradient'] = animation_arg['gradient']
        animation['timer'] = _ba.Timer(0.01, ba.Call(self.update_node_animation, node))
        self.node_animations.append(animation)

    def update_node_animation(self, node):
        for animation in self.node_animations:
            if animation != None and animation['node'] == node:
                color_index = animation['color_index']
                color1 = animation['cur_color']
                if color1 == None:
                    color1 = get_color(animation['colors'][color_index])
                    if color1 == None:
                        self.handlemessage(ColorNotFoundMessage())
                        self.stop_node_animation(node, animation['color_parameter'])
                        return
                if not self.set_node_color(node, animation['color_parameter'], color1, k=animation['k']):
                    return
                animation['color_index'] = (color_index + 1) % len(animation['colors'])
                color2 = get_color(animation['colors'][color_index])
                if color2 == None:
                    self.handlemessage(ColorNotFoundMessage())
                    self.stop_node_animation(node, animation['color_parameter'])
                    return
                animation['cur_color'] = color2
                time_interval = animation['time_intervals'][animation['time_interval_index']]
                if hasattr(self, 'globalsnode') and hasattr(self.globalsnode, 'slow_motion') and self.globalsnode.slow_motion:
                    time_interval /= 3.0
                animation['time_interval_index'] = (animation['time_interval_index'] + 1) % len(animation['time_intervals'])
                if animation['gradient']:
                    if not self.set_node_animation(node, animation['color_parameter'], color1, color2, time_interval, k=animation['k']):
                        return
                animation['timer'] = _ba.Timer(time_interval, ba.Call(self.update_node_animation, node))
